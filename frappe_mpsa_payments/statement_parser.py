	...
	for row in range(header_row + 1, sheet.nrows):
		...
		payment_rows.append(
			StatementRow(
				...
				completion_time=completed_at.strftime("%Y-%m-%d %H:%M:%S") if completed_at else "",
				amount=amount,
				billrefnumber=_to_clean_id(_cell(sheet, row, columns.get("account_no", -1))),
				transactiontype=_to_text(_cell(sheet, row, columns.get("reason_type", -1))).strip(),
				msisdn=msisdn,
				firstname=firstname,
				middlename=middlename,
				lastname=lastname,
				currency=_to_text(_cell(sheet, row, columns.get("currency", -1))).strip(),
				row_index=row,
			)
		)
...
def parse_statement(file_path: str) -> ParsedStatement:
	...
	try:
		...
		raise StatementParseError("The 'xlrd' package is required to read M-Pesa .xls statements.") from exc
