...
def test_blank_rows_are_not_counted_as_data_rows():
    ...
    assert result.total_rows == len(make_fixture.PAYMENTS) * 2 + len(
        make_fixture.SETTLEMENTS
    )
...
def test_amount_sum_matches_header_total(parsed):
    ...
    assert (
        round(sum(row.amount for row in parsed.payment_rows), 2)
        == parsed.header_total_paid_in
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "25470****111 - ANN WANJIRU KAMAU",
            ("25470****111", "ANN", "WANJIRU", "KAMAU"),
        ),
        # Multiple middle tokens are joined.
        (
            "25470****111 - ANN WANJIRU MUTHONI KAMAU",
            ("25470****111", "ANN", "WANJIRU MUTHONI", "KAMAU"),
        ),
