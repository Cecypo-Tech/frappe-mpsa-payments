...
class TestB2CPaymentDisbursement(FrappeTestCase):
    def setUp(self):
        # Document refuses to be constructed bare - it needs at least a
        # doctype to hang its fields off. new_doc is the supported way to get
        # an unsaved one.
        self.payment_disbursement = frappe.new_doc("B2C Payment Disbursement")

    def test_uuid_generation(self):
        """The request document is what mints conversation ids, not this one."""
        request = frappe.new_doc("B2C Disbursement Request")

        generated = request._generate_uuid_v4()

        self.assertEqual(len(generated), 36)
        self.assertNotEqual(generated, request._generate_uuid_v4())
    ...
    @patch("frappe.get_cached_value")
    def test_set_missing_values_sets_source_exchange_rate(
        self, mock_get_cached_value, mock_get_exchange_rate
    ):
        """Test that set_missing_values sets source_exchange_rate if missing."""
        mock_get_exchange_rate.return_value = 110.0
    ...
    def test_get_mpesa_settings_success(self, mock_get_doc):
        ...
        # The gateway name is the mode of payment with its "Mpesa-" prefix off.
        self.payment_disbursement.mode_of_payment = "Mpesa-Test Gateway"
    ...
    @patch("frappe.get_doc")
    def test_get_mpesa_settings_not_found(self, mock_get_doc):
        """A gateway with no settings behind it has to stop the disbursement."""
        self.payment_disbursement.mode_of_payment = "Mpesa-Missing Gateway"
        mock_get_doc.side_effect = frappe.DoesNotExistError

        with self.assertRaises(frappe.ValidationError) as context:
            self.payment_disbursement._get_mpesa_settings()

        self.assertIn("Mpesa-Missing Gateway", str(context.exception))
