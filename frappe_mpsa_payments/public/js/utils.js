...
mpesa_qp.show_dialog = function (
	...
	const dialog = new frappe.ui.Dialog({
		...
		fields: [
			{
				...
				options: mpesa_summary_html(frm, outstanding),
			},
		...
		primary_action: () => process_mpesa(frm, dialog, outstanding),
		// secondary_action_label: __("Request Payment"),
		// secondary_action: () => mpesa_show_request_dialog(frm),
	});
	...
	dialog.search_term = "";
	// dialog.selected = initial_selections;
...
function mpesa_summary_html(frm, outstanding) {
...
function mpesa_render_list(dialog) {
	...
	if (payments.length === 0 && (dialog.search_term || "").length >= 3) {
		html += `<div class="mpesa-no-results"><i class="fa fa-search text-muted"></i><p>${__("No payments match your search")}</p></div>`;
	} else if (payments.length === 0) {
		html += `<div class="mpesa-search-prompt"><i class="fa fa-hand-o-up text-muted"></i><p>${__("Enter a search term above to find payments")}</p></div>`;
	} else {
		html += `
					...
					<div class="mpesa-item-info">
						<div class="mpesa-item-primary">
							...
							<span class="mpesa-amount ${is_over && !is_exact ? "text-warning" : ""}">
								...
								${is_over && !is_exact ? `<i class="fa fa-exclamation-triangle" title="${__("Exceeds outstanding")}"></i>` : ""}
							</span>
						</div>
...
function invoice_payment_options(dialog, fully_paid) {
...
// TODO: check on create_payment_request apps.erpnext.erpnext.accounts.doctype.pos_invoice.pos_invoice.create_payment_request
function mpesa_show_request_dialog(frm) {
	const outstanding = flt(frm.doc.outstanding_amount || 0);
	frappe.call({
		method: "frappe_mpsa_payments.frappe_mpsa_payments.api.mpesa_quick_pay.pos_quick_pay_mpesa_process",
		args: { action: "get_customer_phone", doctype: frm.doctype, customer: frm.doc.customer },
		callback(r) {
			...
			const req = new frappe.ui.Dialog({
				...
				fields: [
					...
					},
					{
						...
						reqd: 1,
						default: phone,
						description: __("Format: 0727870777 or 254727870777"),
					},
				],
						...
						callback(r) {
							if (r.message && r.message.success) {
								...
								frappe.msgprint({
									...
									message: `<p>${__("Sent to")} <strong>${values.phone_number}</strong></p>
                                                <a href="/app/payment-request/${r.message.payment_request}"
...
mpesa_qp.inject_btn_styles = function () {
	...
	$(`<style id="mpesa-pos-styles">
		...
		.mpesa-summary-panel {
			margin-top: 8px;
			border: 1px solid rgba(0,166,80,.3);
			border-radius: 6px;
			overflow: hidden;
			font-size: 12px;
		}
		...
		.mpesa-summary-item {
			...
			gap: 6px;
			padding: 5px 10px;
			border-top: 1px solid rgba(0,166,80,.15);
		}
...
function mpesa_inject_styles() {
	...
	$(`<style id="mpesa-form-styles">
        ...
        .mpesa-count-badge {
            ...
            padding:8px 12px; background:rgba(0,166,80,.1);
            border-radius:6px; color:#00a650; font-size:13px; white-space:nowrap;
        }
        ...
        .mpesa-totals-bar {
            ...
            border:1px solid var(--border-color); flex-wrap:wrap; align-items:center;
        }
